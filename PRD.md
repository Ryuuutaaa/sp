...
members ◄──────────────────────────────────────────┐        │
  ...
  │ 1:N                1:N              1:N        │        │
  ▼                    ▼                ▼          │        │
savings_transactions  loans        shu_distributions│        │
  │                    │                           │        │
  │ N:1                │ 1:N                       │        │
  ▼                    ▼                           │        │
savings_types      installments                    │        │
                                                   │        │
cash_transactions (polymorphic ref) ───────────────┘        │
                                                            │
...
|---|---|---|
| `users.member_id` → `members.id` | 1:1 (nullable) | User role `anggota` terhubung ke member. Role lain nullable |
| `members.verified_by` → `users.id` | N:1 | Admin/Pengurus yang memverifikasi anggota |
| `savings_transactions.member_id` → `members.id` | N:1 | Transaksi simpanan milik anggota mana |
| `savings_transactions.savings_type_id` → `savings_types.id` | N:1 | Jenis simpanan (pokok/wajib/sukarela) |
| `savings_transactions.created_by` → `users.id` | N:1 | Teller/Admin yang input transaksi |
| `loans.member_id` → `members.id` | N:1 | Pinjaman milik anggota mana |
| `loans.approved_by` → `users.id` | N:1 | Admin yang approve pinjaman |
| `loans.disbursed_by` → `users.id` | N:1 | Teller/Admin yang mencairkan |
| `installments.loan_id` → `loans.id` | N:1 | Angsuran milik pinjaman mana |
| `installments.paid_received_by` → `users.id` | N:1 | Teller yang terima pembayaran |
| `cash_transactions.created_by` → `users.id` | N:1 | Siapa yang input kas |
| `cash_transactions.reference_id` → (polymorphic) | N:1 | Merujuk ke `savings_transactions`, `loans`, atau `installments` via `reference_type` |
| `shu_distributions.member_id` → `members.id` | N:1 | SHU untuk anggota mana |
